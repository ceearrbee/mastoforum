/**
 * Minimal, dependency-free SHA-256. Used as a fallback for the PKCE S256
 * challenge when `crypto.subtle` is unavailable (i.e. an insecure context such
 * as plain-HTTP LAN access). Mastodon requires `code_challenge_method=S256`, so
 * we can't downgrade to `plain` — we compute the digest ourselves instead.
 *
...
/** SHA-256 of the given bytes, returned as 32 raw bytes. */
